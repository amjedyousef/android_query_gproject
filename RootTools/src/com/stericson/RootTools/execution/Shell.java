...
public class Shell {
    ...
    public static Shell startNewRootShell(int timeout, int retry) throws IOException, TimeoutException, RootDeniedException
    {
    ...
    public static Shell startRootShell(int timeout, int retry) throws IOException, TimeoutException, RootDeniedException {
		...
		if (rootShell == null)
		{
			rootShell = startNewRootShell(timeout, retry);
		} else
		{
    ...
    public void kill()
	{
		...
		{
			// Get process PID
			String processDescription = proc.toString();
			int pid = Integer.parseInt(processDescription.split("pid=")[1].split("]")[0]);

			// Run 'kill' (HACK!)
			CommandCapture command = new CommandCapture(-1, "kill -9 " + pid);
